...
void coordinates34_2(Point2d src, float angle, Point2d & dst)
{
	dst.x = src.x*cos(angle) - src.y*sin(angle);
	dst.y = src.x*sin(angle) + src.y*cos(angle);
}
void coordinates34_1(Point2f src, float angle, Point2f & dst)
{
	dst.x = src.x*cos(angle) - src.y*sin(angle);
	dst.y = src.x*sin(angle) + src.y*cos(angle);
...
void J_Rotate34_2(Mat src, int x1, int y1, int x2, int y2, float angle, Mat dst,
	int & l1, int & t1, int & r1, int & b1, Point & pt5, Point & pt6, Point & pt7, Point & pt8)
{
	...
	coordinates34_2(leftTop, angle, transLeftTop);
	coordinates34_2(rightTop, angle, transRightTop);
	coordinates34_2(leftBottom, angle, transLeftBottom);
	coordinates34_2(rightBottom, angle, transRightBottom);
...
// 旋转中心，坐标为车牌中心，旋转区域车牌区域
void J_Rotate34_1(Mat src, int x1, int y1, int x2, int y2, float angle, Mat dst,
	int & l1, int & t1, int & r1, int & b1, Point & pt5, Point & pt6, Point & pt7, Point & pt8)
{
	Point center;
	...
	coordinates34_1(leftTop, angle, transLeftTop);
	coordinates34_1(rightTop, angle, transRightTop);
	coordinates34_1(leftBottom, angle, transLeftBottom);
	coordinates34_1(rightBottom, angle, transRightBottom);
...
void DrawLine34_1(cv::Mat img, Point pt1, Point pt2, Point pt3, Point pt4)
{
	int thick = 1;
...
void PutText34_1(cv::Mat img, Point pt1, Point pt2, Point pt3, Point pt4)
{
	...
	sprintf(file, "1");
	pt.x = pt1.x + 50; pt.y = pt1.y + 50;
	putText(img, file, pt, 1, 1, green);

	sprintf(file, "2");
	pt.x = pt2.x + 50; pt.y = pt2.y + 50;
	putText(img, file, pt, 1, 1, green);

	sprintf(file, "3");
	pt.x = pt3.x + 50; pt.y = pt3.y + 50;
	putText(img, file, pt, 1, 1, green);

	sprintf(file, "4");
	pt.x = pt4.x + 50; pt.y = pt4.y + 50;
	putText(img, file, pt, 1, 1, green);
...
int test34(int argc, char *argv[])
{
	...
	for (int i = 0; i < v_img_.size(); i++)
	{
		...
		for (int j = 0; j < 2; j++)
		{
			...
			pt1.x = x1; pt1.y = y1; pt2.x = x2; pt2.y = y2;
			int left = 0; int top = 0; int right = 0; int bottom = 0;
			J_Rotate34_1(img, x1, y1, x2, y2, radian, dst, left, top, right, bottom, pt5, pt6, pt7, pt8);
			pt3.x = left; pt3.y = top; pt4.x = right; pt4.y = bottom;
			...
			finWrite << str1 << " " << label << " " << left << " " << top << " " << right << " " << bottom << " " <<
			   ...
			   Mat drawimg = dst.clone();
				rectangle(drawimg, pt3, pt4, Scalar(0, 0, 255));
				DrawLine34_1(drawimg, pt5, pt6, pt7, pt8);
				PutText34_1(drawimg, pt5, pt6, pt7, pt8);
